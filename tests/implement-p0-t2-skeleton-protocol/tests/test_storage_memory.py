...
def test_sm_01_lpush_and_lrange(memory_store):
    ...
    # 模拟推入 3 条数据
    memory_store.lpush(key, {"id": 1, "text": "first"})
    memory_store.lpush(key, {"id": 2, "text": "second"})
    memory_store.lpush(key, {"id": 3, "text": "third"})
    
    # 获取所有数据
    items = memory_store.lrange(key, 0, -1)
...
def test_sm_02_ltrim_truncation(memory_store):
    ...
    for i in range(5):
        memory_store.lpush(key, {"id": i})
        
    # 当前列表从左到右: id=4, 3, 2, 1, 0
    # 截断，只保留最新的 3 条 (索引 0, 1, 2)
    memory_store.ltrim(key, 0, 2)
...
def test_sm_03_append_hot_memory_with_max_rounds(memory_store):
    ...
    result = memory_store.append_hot_memory(
        logic_id, session_id,
...
def test_sm_04_runtime_state_persistence(memory_store):
    ...
    # 存入状态
    memory_store.persist_runtime_state(logic_id, session_id, state)
    
    # 读取状态
    loaded_state = memory_store.load_runtime_state(logic_id, session_id)
    
    assert loaded_state == state
    assert loaded_state is not state  # 应该是深拷贝或新建的字典，不共享内存地址

    # 读取不存在的状态
    empty_state = memory_store.load_runtime_state("unknown", "unknown")
    assert empty_state == {}
