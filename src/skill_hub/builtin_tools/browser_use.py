        ...
        self.capability = CapabilityDescription(
            ...
            input_schema={
                ...
                "properties": {
                    ...
                    "probe_only": {"type": "boolean", "description": "如果为 True，仅探测运行环境是否可用，不执行实际访问。"},
                    "headless": {"type": "boolean", "description": "是否以无头模式运行浏览器，默认为 True。"},
                    "browser_type": {
                        "type": "string", 
                        "enum": ["chromium", "firefox", "webkit"],
                        "description": "浏览器引擎类型，默认为 chromium。"
                    },
                    "click_selector": {"type": "string", "description": "页面加载后要点击的 CSS 选择器（可选）。"},
                    "wait_ms": {"type": "integer", "description": "页面加载或动作执行后的等待时间（毫秒）。"},
                    "screenshot_dir": {"type": "string", "description": "截图保存的目录路径。"},
                    "actions": {
                        "type": "array",
                        "description": "要在页面上执行的动作序列（点击、输入、按键等）。",
                        "items": {
                            "type": "object",
                            "properties": {
                                "type": {"type": "string", "description": "动作类型，如 click, type, key_press, wait, screenshot, goto。"},
                                "url": {"type": "string", "description": "goto 动作的目标 URL。"},
                                "selector": {"type": "string", "description": "动作作用的 CSS 选择器。"},
                                "text": {"type": "string", "description": "type 动作要输入的文本。"},
                                "key": {"type": "string", "description": "key_press 动作要按下的键名。"},
                                "wait_ms": {"type": "integer", "description": "动作后的等待时间。"},
                                "path": {"type": "string", "description": "screenshot 动作保存的路径。"},
                            },
                            "additionalProperties": True,
                        },
                    },
                    "extract_text": {"type": "boolean", "description": "是否提取页面的纯文本内容。"},
                    "max_text_chars": {"type": "integer", "description": "提取文本的最大字符数限制。"},
                    "extract_dom": {"type": "boolean", "description": "是否提取简化后的 DOM 树结构。"},
                    "max_dom_chars": {"type": "integer", "description": "提取 DOM 的最大字符数限制。"},
                    "extract_links": {"type": "boolean", "description": "是否提取页面中的所有超链接。"},
                    "max_links": {"type": "integer", "description": "提取链接的最大数量限制。"},
                },
                "additionalProperties": True,
            },
            output_schema={
                "type": "object",
                "properties": {
                    "accepted": {"type": "boolean", "description": "请求是否被接受并开始执行。"},
                    "execution_mode": {"type": "string", "description": "执行模式说明。"},
                    "runtime": {"type": "object", "description": "浏览器运行时的元数据信息。"},
                    "url": {"type": "string", "description": "请求访问的原始 URL。"},
                    "url_before": {"type": "string", "description": "动作执行前的页面 URL。"},
                    "url_after": {"type": "string", "description": "动作执行后的最终页面 URL。"},
                    "title_before": {"type": "string", "description": "动作执行前的页面标题。"},
                    "title_after": {"type": "string", "description": "动作执行后的最终页面标题。"},
                    "screenshot_path": {"type": "string", "description": "生成的截图文件的本地路径。"},
                    "page_text": {"type": "string", "description": "从页面提取的纯文本内容。"},
                    "dom_summary": {"type": "string", "description": "从页面提取的简化 DOM 结构。"},
                    "links": {"type": "array", "description": "从页面提取的链接列表。"},
                    "blocked_by_captcha": {"type": "boolean", "description": "是否被验证码拦截。"},
                    "block_reason": {"type": "string", "description": "拦截或失败的具体原因描述。"},
                },
                "additionalProperties": True,
            },
