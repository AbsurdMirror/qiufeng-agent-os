...
class ModelOutputParser:
    """
    模型输出解析器 (Model Output Parser)
    封装了对模型返回文本的 JSON 解析、Schema 校验以及工具调用解析。
    """
    ...
    def parse_message_content(
        ...
        *,
        ...
        try:
            ...
            self._record(
                trace_id,
                "output_parser.schema_validation_failed",
                {
                    "content": content_str,
                    "error": str(exc),
                    "schema": self._schema_name(schema),
                },
                "WARNING"
            )
    ...
    def convert_litellm_tool_calls(
        ...
        *,
        ...
        for index, item in enumerate(tool_calls):
            try:
                invocation = self._coerce_tool_invocation(item, tool_index=index)
                ...
                payload = self._parse_tool_arguments(
                    invocation.function.arguments,
                    tool_name=capability_id,
                    tool_index=index,
                )
                
                payload_error = self._validate_payload_by_schema(payload, capability.input_schema)
                ...
                self._record(
                    trace_id,
                    "output_parser.tool_call_validation_failed",
                    exc.to_dict(),
                    "WARNING"
                )
                raise
    ...
    def _validate_payload_by_schema(
        ...
        if isinstance(properties, Mapping):
            for key, prop in properties.items():
                ...
                if expected is None:
                    continue
                if not self._matches_json_type(payload[key], expected):
    ...
    def _schema_name(self, schema: type[T] | Mapping[str, object]) -> str | None:
        if isinstance(schema, type) and issubclass(schema, BaseModel):
            return schema.__name__
        return None


# 为了保持向下兼容，保留原始函数作为代理（可选，但推荐在迁移期间保留）
_default_parser = ModelOutputParser()

def parse_message_content(*args, **kwargs):
    return _default_parser.parse_message_content(*args, **kwargs)

def convert_litellm_tool_calls(*args, **kwargs):
    return _default_parser.convert_litellm_tool_calls(*args, **kwargs)
